...
export default function PremiumCard({ data = {} }) {
  const {
    ...
    upiId = "rahul@ybl",
    savingsRatio = 5.1, // total received / total paid
  } = data;
    ...
    <div className="glass-card gradient-border rounded-2xl p-6 h-full">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          ...
          { label: "Total Received", value: `₹${totalReceived}`, color: "text-emerald-400" },
          { label: "Savings Ratio", value: `${savingsRatio}x`, color: "text-amber-400" },
        ].map((s) => (
      ...
      <div className="flex items-center gap-3 bg-emerald-500/8 border border-emerald-500/20 rounded-xl px-4 py-3 mb-5">
        ...
        <p className="text-emerald-300 text-xs">
          You've received <span className="font-semibold">₹{totalReceived - totalPaid} more</span> than you paid in premiums.
        </p>
      </div>
